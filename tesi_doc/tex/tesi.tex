...
\begin{comment}
Le teorie di gauge non abeliane presentano il fenomeno del confinamento di colore: il potenziale d'interazione fra due cariche di colore cresce linearmente nel limite di lunghe distanze. Il tubo di flusso formato dalle linee di forza del potenziale può essere modellato come una stringa vibrante: questo modello è detto Effective String Theory (EST). È un modello molto predittivo a basse temperature e lunghe distanze, ma che perde affidabilità ad alte temperature, vicino al punto critico della transizione di deconfinamento. Nel regime critico, una congettura di Svetisky e Yaffe prevede che in una teoria di gauge (2+1) dimensionale, con transizione di deconfinamento di secondo ordine, i dettagli fini delle interazioni possono essere ignorati e il sistema è nella stessa classe di universalità del modello di Ising 2D, in quanto entrambi hanno la stessa simmetria Z2 e dimensionalità. Si è scritto un programma per simulare la teoria di Yang-Mills in tre dimensioni a temperatura finita regolarizzata su reticolo usando l'algoritmo Heat-Bath di Cabibbo e Marinari per verificare la congettura nel caso del gruppo di gauge Sp(2). Si è misurato il correlatore del loop di Polyakov (il parametro d'ordine della transizione di deconfinamento) e si è verificato che ha lo stesso andamento del correlatore fra spin del modello di Ising 2D, a corte e lunghe distanze, suggerendo che la congettura è vera per la teoria (2+1) dimensionale di Yang-Mills con gruppo di gauge Sp(2). 
\end{comment}

\begin{abstract}
	Non abelian gauge theories exhibit color confinement: the potential between two color charges is linearly
	rising in the large distance limit. An Effective String Theory (EST) exists which describes the flux tube formed by
	the field lines of the potential as a vibrating string. This is an effective model with very good predictive power at
	low temperatures and large distances, but it is not reliable in the high temperature regime, near the deconfinement critical point. Here,
	the Svetisky-Yaffe conjecture predicts that a $(2+1)$ dimensional gauge theory is in the same universality class
	of the 2D Ising model if the deconfinement phase transition is second order, 
	because the fine details of the interactions can be neglected and the two systems share the same
	$\mathbb{Z}_2$ symmetry and dimensionality. 
	...
	spin-spin correlator of the 2D Ising model for short and large distances,
	hinting that the conjecture is true for the $(2+1)$ Yang-Mills
...
\begin{figure}[h]
	\centering
		\includegraphics[width=0.7\textwidth]{susc_all_l/suscplot_nt=6.png} 

		\includegraphics[width=0.7\textwidth]{susc_all_l/suscplot_nt=8.png}	
...
\begin{figure}[h]
	\centering
	\begin{tabular}{c c}
		\includegraphics[width=0.45\textwidth]{susc/suscplot_nt=8_l=40.png} &
		\includegraphics[width=0.45\textwidth]{susc/suscplot_nt=8_l=60.png}
		\\
		\includegraphics[width=0.45\textwidth]{susc/suscplot_nt=8_l=80.png} &
		\includegraphics[width=0.45\textwidth]{susc/suscplot_nt=8_l=100.png}
	\end{tabular}	
	\caption[Susceptibility peak for $N_t = 8$]{Peak of the susceptibility over spatial volume for
	$N_t = 8$. From left to right and top to bottom, they are for $N_s = 40, 60, 80, 100$.}
	\label{fig:susc_peaks_nt8}
\end{figure}
\end{comment}
