...
md"""
All Ls: $(@bind all_ls CheckBox(default=true))

All Nts: $(@bind all_nts CheckBox(default=true))
"""
...
getkeys(df, k) = getproperty.(keys(df), k)
...
begin
	L_values = sort(unique(getkeys(dfs, :l)))
	Nt_values = sort(unique(getkeys(dfs, :nt)))
	L_md = @bind l_list MultiSelect(L_values, default = L_values)
	Nt_md = @bind nt_list MultiSelect(Nt_values, default = Nt_values)
	if !all_ls && !all_nts
		md"""L =
		$L_md
...
begin
	...
	for ((k, df), marker) in zip(pairs(dfs), Iterators.cycle(markers))
		L = k.l
		Nt = k.nt
		L ∉ Ls && continue
		Nt ∉ Nts && continue
